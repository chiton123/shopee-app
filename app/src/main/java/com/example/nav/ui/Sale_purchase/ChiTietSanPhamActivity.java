...
public class ChiTietSanPhamActivity extends AppCompatActivity {
    TextView txttenchitiet, txtgiachitiet, txtsoluongconlai;
    ...
    int idowner = 0;
    String owner_name = "";
    // mang cho spinner với số lượng hàng còn trong kho
...
//        spinner.setAdapter(adapter);
                        ...
                        if(MainActivity.manggiohang.size() > 0){
                            ...
                            for(int i=0 ; i < MainActivity.manggiohang.size(); i++){
                                if(MainActivity.manggiohang.get(i).getIdsp() == id) {
                                    exists = true;
                                    int a = soluongtoida - soluongmua - MainActivity.manggiohang.get(i).getSoluongsp();
                                    if(a >= 0){
                                        MainActivity.manggiohang.get(i).setSoluongsp(MainActivity.manggiohang.get(i).getSoluongsp() + soluongmua);
                                        ...
                                        int conlai = soluongtoida - MainActivity.manggiohang.get(i).getSoluongsp();
                                        Toast.makeText(getApplicationContext(), "Shop chỉ còn " + conlai, Toast.LENGTH_SHORT).show();
                                    }
                                }
                            }
                            if(exists == false){
                                int soluong = Integer.parseInt(spinner.getSelectedItem().toString());
                                int a = soluongtoida - soluong;
                                if(a >= 0){
                                    long giamoi = soluong * giachitiet;
                                    MainActivity.manggiohang.add(new GioHang(id, tenchitiet, giamoi, hinhanhchitiet, soluong, soluongtoida));
                                    Toast.makeText(getApplicationContext(), "Đã thêm", Toast.LENGTH_SHORT).show();
                                    kiemtra = 1;
                                    if(kiemtra == 1){
                                        Intent intent = new Intent(getApplicationContext(), GioHangActivity.class);
    ...
    private void getThongTin() {
        ...
        try{
            ...
            soluongtoida = sp.getSoluong() - sp.getSoluongdaban();
            txtsoluongconlai.setText("Số sản phẩm còn lại: "+ soluongtoida);
        }catch (NullPointerException e){
            Toast.makeText(getApplicationContext(), "Lỗi NullPointerException", Toast.LENGTH_SHORT).show();
        }
    ...
    private void anhxa() {
        ...
        txtsoluongconlai = (TextView) findViewById(R.id.soluongconlai);
